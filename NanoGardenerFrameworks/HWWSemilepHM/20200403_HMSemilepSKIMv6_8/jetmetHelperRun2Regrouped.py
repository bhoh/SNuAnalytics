...
from LatinoAnalysis.NanoGardener.modules.jetmetUncertainties import *
...
def createJMECorrector(isMC=True, dataYear=2016, runPeriod="B", jesUncert="Total", redojec=False, jetType = "AK4PFchs", noGroom=False, metBranchName="MET", applySmearing=True, isFastSim=False):
    ...
    if isMC and not isFastSim:
        ...
        try:
            ...
            print "No tag for ->",dataYear + runPeriod
            return
    jmeUncert_ = [x for x in jesUncert.split(",")]

    print "jes uncertainies"
    print jmeUncert_
...
#incl    def __init__(self, era, globalTag, jesUncertainties = [ "Total" ], archive=None, jetType = "AK8PFPuppi", redoJEC=False, noGroom=False, jerTag="", jmrVals = [], jmsVals = [],\ isData=False, applySmearing=True):ude jmeCorrections() in the list of modules to run.
###
