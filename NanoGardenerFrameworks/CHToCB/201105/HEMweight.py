...
class HEMweight(Module):
    ...
    def __init__(self, isData, dataYear, jetColl="Jet", cmssw = 'Full2016', seed=65539):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        if self.HEMPtScale:
          ...
          self.out.branch(self.jetColl+'_pt_HEM', 'F', lenVar='n'+self.jetColl) ##fix to NanoAOD format
    ...
    def analyze(self, event):
        ...
        if self.isData:
          ...
          if self._hasHEMJet(jet_coll):
            ...
            pass
          if run_period == 3 or run_period == 4:
            Jet_pt_HEM = self._scaleHEMJetPt(jet_coll)
          else:
            Jet_pt_HEM = self._scaleHEMJetPt(jet_coll, False)
        if self.doHEMweight:
          self.out.fillBranch('HEMweight', HEMweight_)
          self.out.fillBranch('RunPeriod_HEM',run_period)
        if self.HEMPtScale:
          self.out.fillBranch(self.jetColl+'_pt_HEM', Jet_pt_HEM)
          # getattr(self,'HEM%sPtScale'%self.jetColl) is self.doHEMweight and not self.isData
    ...
    def _scaleHEMJetPt(self, jet_coll_, scale=True):
        HEMJetPtScale = []
        ...
        for jet in jet_coll_:
          ...
          eta = jet["eta"]
          phi = jet["phi"]
          if pt > 15. and -1.57 < phi < -0.87 and scale:
              if -2.5 < eta < -1.3:
                  HEMJetPtScale.append(0.8*pt) # scale down 20% 
              elif -3.0 < eta <= -2.5:
                  HEMJetPtScale.append(0.65*pt) # scale down 35%
              else:
                  HEMJetPtScale.append(pt)
          else:
            HEMJetPtScale.append(pt)
