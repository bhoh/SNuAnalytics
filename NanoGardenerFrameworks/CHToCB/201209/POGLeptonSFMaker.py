...
class POGLeptonSFMaker(Module):
    '''
    Produce branches with recoSF, IDIsoSF, totSF
    ''' 

    def __init__(self, cmssw, WP_path, branch_map=''):
        self.cmssw = cmssw
        ...
        print('POGLeptonSFMaker: making scale factors for analysis of ' + self.cmssw)
            ...
            for SFkey in self.ElectronWP[self.cmssw]['TightObjWP'][wp]:
                ...
                if SFkey == 'wpSF':
                    ...
                    for rpr in self.ElectronWP[self.cmssw]['TightObjWP'][wp]['wpSF']:
                        ...
                        wp_file = self.open_root(cmssw_base + '/src/' + self.ElectronWP[self.cmssw]['TightObjWP'][wp]['wpSF'][rpr])
                        self.SF_dict['electron'][wp]['wpSF']['data'].append(self.get_root_obj(wp_file, 'EGamma_SF2D'))
                        wp_file.Close()
            ...
            for SFkey in self.MuonWP[self.cmssw]['TightObjWP'][wp]:
                if SFkey == 'tkSF':
                    ...
                    for rpr in self.MuonWP[self.cmssw]['TightObjWP'][wp]['tkSF']:
                        ...
                        self.SF_dict['muon'][wp]['tkSF']['data'].append(self.get_root_obj(tk_file, 'ratio_eff_vtx_dr030e030_corr'))
                        tk_file.Close()
                if SFkey == 'idSF':
                    ...
                    for rpr in self.MuonWP[self.cmssw]['TightObjWP'][wp]['idSF']:
                        ...
                        if split_file[-1] == 'root':
                            ...
                            #XXX for 2018
                            if self.cmssw == 'Full2018v7':
                                self.SF_dict['muon'][wp]['idSF']['data'].append(self.get_root_obj_TH2D(data_file, 'NUM_TightID_DEN_TrackerMuons_pt_abseta'))
                            elif self.cmssw == 'Full2017v7':
                                self.SF_dict['muon'][wp]['idSF']['data'].append(self.get_root_obj_TH2D(data_file, 'NUM_TightID_DEN_genTracks_pt_abseta'))
                            elif self.cmssw == 'Full2016v7':
                                self.SF_dict['muon'][wp]['idSF']['data'].append(self.get_root_obj_TH2D(data_file, 'NUM_TightID_DEN_genTracks_eta_pt'))
                            else:
                                raise Exception('POGLeptonSFMaker, check self.SF_dict for muon ID')
                            data_file.Close()
    ...
    def get_root_obj_TH2D(self, root_file, obj_name):
        r_obj = root_file.Get(obj_name)
        root_file.Print()
        if not r_obj.__nonzero__(): raise NameError('Root Object ' + obj_name + ' not found')
        return copy.deepcopy(r_obj)
    ...
    def get_hist_VnE(self, hist, x, y):
        ...
        if hx < xmin: hx = xmin + 0.001
        if hx > xmax: hx = xmax - 0.001

        if hy < ymin: hy = ymin + 0.001
        if hy > ymax: hy = ymax - 0.001
    ...
    def trunc_kin(self, pdgId, pt, eta):
        ...
        #else: raise ValueError('POGLeptonSFMaker trunc_kin: non lepton detected in what should be lepton collection')
        return pt_t, eta_t
    ...
    def get_idIso_SF(self, pdgId, lep_pt, lep_eta, nvtx, wp, run_period):
        ...
        tkSF_sys = 1.
        if abs(pdgId) == 11:
            tkSF, tkSF_err = self.get_hist_VnE(self.SF_dict[kin_str][wp]['wpSF']['data'][run_idx], eta, pt)
            tkSF_sys = 0.0
        ...
        if abs(pdgId) == 13:
            ...
            if not self.SF_dict['muon'][wp]['idSF']['isRoot'][run_idx]:
                ...
                if self.cmssw == 'Full2018v7':
                    tkSF_id, tkSF_err = self.get_hist_VnE(self.SF_dict[kin_str][wp]['idSF']['data'][run_idx], pt, abs(eta))
                elif self.cmssw == 'Full2017v7':
                    tkSF_id, tkSF_err = self.get_hist_VnE(self.SF_dict[kin_str][wp]['idSF']['data'][run_idx], pt, abs(eta))
                elif self.cmssw == 'Full2016v7':
                    tkSF_id, tkSF_err = self.get_hist_VnE(self.SF_dict[kin_str][wp]['idSF']['data'][run_idx], eta, pt)
                else:
                    pdgId, pt, eta, tkSF_id, tkSF_err = self.get_hist_VnE(self.SF_dict[kin_str][wp]['idSF']['data'][run_idx], eta, pt)

                tkSF_id_up = tkSF_err
                tkSF_id_dwn = tkSF_err

                #tkSF_id_sys = math.sqrt( float(dot[8])**2 + float(dot[9])**2 + float(dot[1pdgId, pt, eta, 0])**2 + float(dot[11])**2)
                #tkSF_id_sys /= mc_id
            ...
            if self.SF_dict[kin_str][wp]['hastthMvaSF']:
               ...
               try:
                   ...
                   tkSF_up  = tkSF_id * tkSF_iso
                   tkSF_dwn = tkSF_id * tkSF_iso
                   tkSF = tkSF_id*tkSF_iso
    ...
    def analyze(self, event):
        ...
        for iLep in range(nLep):
           ...
           if abs(lepton_col[iLep]['pdgId']) == 11:
              ...
              for wp in self.ElectronWP[self.cmssw]['TightObjWP']:
                  ...
                  #BHO
                  #if (idiso_sf_up + idiso_sf) > 1.2*idiso_sf or abs(idiso_sf - idiso_sf_dwn) < 0.8*idiso_sf:
                  #    print(pdgId, pt, eta, idiso_sf, idiso_sf + idiso_sf_up, idiso_sf - idiso_sf_dwn)
                  # SF 1 /pm 1 for gap region
        ...
        for key in el_wp_var:
            #BHO debug
            if el_wp_var[key]==0:
                raise Exception('BHO debug !!!!!, el_wp_var[key]==0')
            self.out.fillBranch('Lepton_tightElectron_' + key, el_wp_var[key])
        for key in mu_wp_var:
            #BHO debug
            if mu_wp_var[key]==0:
                raise Exception('BHO debug !!!!!, mu_wp_var[key]==0')
