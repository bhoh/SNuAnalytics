...
class PtCorrApplier(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        #    print br.GetName()
        backup_colls = []
        print "[jhchoi]Start of beginFile @ PtCorrApplier"
        for br in oBrList:
            bname = br.GetName()
            if 'jecUncert' in bname: print "bname=",bname
        print "[jhchoi]Start of checking oBrList @ PtCorrApplier"
        for br in oBrList:
            bname = br.GetName()
            #print "bname=",bname
            ...
            if re.match('\A'+self.CollTC+'_', bname) and len(bname.split("_"))==2 and "jecUncert" not in bname:
                ...
                if 'jecUncert' in bname : print "append =>",bname
        ...
        print "Using inputTree"
        iBrList = inputTree.GetListOfBranches()
        #print "[jhchoi]check inputBranch has jecUncertTotal"
        #for br in iBrList:
        #    bname = br.GetName()
        #    if 'jecUncert' in bname:
        #        print "jecUncert in input brancheslist ->",bname

        print "------[jhchoi]beforecheck input branch-------- "
        for br in self.out._tree.GetListOfBranches():
            bname = br.GetName()
            if 'jecUncert' in bname : print 'outputBranch beforecheck input branch->',bname
        print "------[jhchoi]start check input branch-------- "
        for br in iBrList:
            bname = br.GetName()
            #print "bname in inputTree=",bname
            btype = Type_dict[br.GetListOfLeaves()[0].GetTypeName()]
            if re.match('\A'+self.CollTC+'_', bname):
                ...
                if 'jecUncert' in bname:print "bname=",bname ##jhchoi 
                self.CollBr[btype].append(bname)
                #print "output branch->",bname+self._suffix
                self.out.branch(bname+self._suffix, btype, lenVar='n'+self.CollTC)
        print "------[jhchoi]aftercheck input branch-------- "
        for br in self.out._tree.GetListOfBranches():
            bname = br.GetName()
            if 'jecUncert' in bname : print 'outputBranch aftercheck input branch->',bname
        print "------[jhchoi]start setup doMET branch-------- "
        ...
        print "------[jhchoi]after setup doMET branch input branch-------- "

        for br in self.out._tree.GetListOfBranches():
            bname = br.GetName()
            if 'jecUncert' in bname:print 'outputBranch->',bname
