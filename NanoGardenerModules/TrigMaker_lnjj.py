...
class TrigMaker_lnjj(Module):
    ...
    def __init__(self, cmssw = 'Full2016', isData = False, keepRunP = False, cfg_path = 'LatinoAnalysis/NanoGardener/python/data/TrigMaker_cfg.py', seeded = False, branch_map=''):
        ...
        if self.isData:
           ...
           self.typeStr = 'MC'
           ## lnjj
           self.NewVar['F'] += ['TriggerEffWeight_2l_lnjj','TriggerEffWeight_2l_lnjj_u','TriggerEffWeight_2l_lnjj_d']
    ...
    def _get_w_lnjj(self, pdgId1, pt1, eta1, pdgId2, pt2, eta2, nvtx, run_p, event_seed=None):
        ...
        for i in range(3): 
           ...
           eff_sgl[i] =  eff[0][i]*eff_gl[0][i]+eff[1][i]*eff_gl[1][i]-eff[0][i]*eff[1][i]*eff_gl[0][i]*eff_gl[1][i]
           #eff_evt[i] = eff_sgl[i] + eff_dbl[i] - eff_sgl[i]*eff_dbl[i] 
           eff_evt[i] = eff_sgl[i] #*(1-eff_dbl[i]) 
        #print eff_dbl , eff_evt        
    ...
    def analyze(self, event):
        ...
        if nLep > 1:
           ...
           temp_evt_lnjj = self._get_w_lnjj(pdgId[0], pt[0], eta[0], pdgId[1], pt[1], eta[1], nvtx, run_p, evt)
           for name in self.NewVar['F']:
               if 'TriggerEffWeight' in name:
                   if '_2l_lnjj' in name:
                       if '_2l_lnjj_d' in name: eff_dict[name] = temp_evt_lnjj[1]
                       elif '_2l_lnjj_u' in name: eff_dict[name] = temp_evt_lnjj[2]
                       else: eff_dict[name] = temp_evt_lnjj[0]
                   elif '_2l' in name:
