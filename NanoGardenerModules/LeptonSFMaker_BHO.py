...
class LeptonSFMaker_BHO(Module):
    '''
    ...
    def __init__(self, cmssw, WP_path = 'LatinoAnalysis/NanoGardener/python/data/LeptonSel_cfg.py', branch_map=''):
        ...
        print('LeptonSFMaker_BHO: making scale factors for analysis of ' + self.cmssw)
    ...
    def trunc_kin(self, pdgId, pt, eta):
        ...
        else: raise ValueError('LeptonSFMaker_BHO trunc_kin: non lepton detected in what should be lepton collection')
        return pt_t, eta_t
    ...
    def analyze(self, event):
        ...
        electron_col   = Collection(event, 'Electron') #BHO
        nLep = len(lepton_col) 
        #try:
           ...
           if abs(lepton_col[iLep]['pdgId']) == 11:
              ...
              for wp in self.ElectronWP[self.cmssw]['TightObjWP']:
                  if not did_reco:
                      ...
                      did_reco = True
                  idiso_sf, idiso_sf_dwn, idiso_sf_up, idiso_sf_sys = self.get_idIso_SF(pdgId, pt, sceta, nvtx, wp, run_period)
