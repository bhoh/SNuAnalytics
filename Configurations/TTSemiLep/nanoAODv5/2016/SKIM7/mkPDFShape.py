        ...
        for variableName in self._variables:
          ...
          for sampleName in self._samples:
            ...
            if sampleName in nuisances['LHEPdfWeight']['samples']:
              ...
              BinVar =[0.]*NbinsX
              nPdfWeights = len(nuisances['LHEPdfWeight']['samples'][sampleName])
              for iVar in range(nPdfWeights):
                histoNameVar  = '{CUT}/{VAR}/histo_{SAMPLE}_{NUIS}V{IDX}Var'.format(CUT=cutName,
                                                                                    VAR=variableName,
                                                                                    SAMPLE=sampleName,
                ...
                for iBin in range(NbinsX):
                  ...
                  #XXX HESSIAN
                  BinVar[iBin] = BinVar[iBin] + (nominal-variant)*(nominal-variant)
              ...
              if nPdfWeights == 33:
                N = 1.
              elif nPdfWeights >= 100:
                N = 99.
              for iBin in range(NbinsX):
                nominal = histoNorm.GetBinContent(iBin+1)
                if nominal != 0.:
                  BinVarRate = math.sqrt(BinVar[iBin])/nominal/N
                  contentUp  = nominal*(1+BinVarRate)
                  contentDown  = nominal*(1-BinVarRate)
                else:
                  BinVarRate = math.sqrt(BinVar[iBin])/N
