...
if True:
  ...
  for syst in jes_syst:
      ...
      nuisances[syst_] = {
          ...
          'samples': dict((skey, ['1.','1.']) for skey in mc),
          'folderUp'   : makeMCDirectory('_jetMETSyst_{SKIM}Up'.format(SKIM=skim_)) if not include_mva else makeMCDirectory_mva('_jetMETSyst_{SKIM}__mvaCHToCB_2016_jetMETSyst_{SKIM}Up'.format(SKIM=skim_))    ,
          'folderDown' : makeMCDirectory('_jetMETSyst_{SKIM}Down'.format(SKIM=skim_)) if not include_mva else makeMCDirectory_mva('_jetMETSyst_{SKIM}__mvaCHToCB_2016_jetMETSyst_{SKIM}Down'.format(SKIM=skim_)),
          'group': 'experimental',
