...
class DatacardFactory:
              ...
              for nuisanceName, nuisance in nuisances.iteritems():
                ...
                if nuisance['type'] in ['lnN', 'lnU']:
                  ...
                  if 'skipCMS' in nuisance and nuisance['skipCMS'] == 1:
                    ...
                    if type(nuisance['rename']) == list:
                      entryName = 'CMS_' + nuisance['rename'][0].replace('Up','').replace('Down','')
                    else:
                    ...
                    for sampleName in processes:
                      if ('all' in nuisance and nuisance ['all'] == 1) or \
                        ...
                        if ('skipCMS' in nuisance.keys()) and nuisance['skipCMS'] == 1:
                          ...
                          if type(nuisance['rename']) == list:
                            suffixOut = ['_CMS_' + nuisance['rename'][0], '_CMS_' + nuisance['rename'][1]]
                          else:
                        ...
                        saved = self._saveNuisanceHistos(cutName, variableName, sampleName, '_' + nuisance['name'], suffixOut, syncronize_stat, symmetrize, nuisance['samples'], nuisance['lnNsamples'] if 'lnNsamples' in nuisance else {}, rescale = rescale)
    ...
    # _____________________________________________________________________________
    def _saveNuisanceHistos(self, cutName, variableName, sampleName, suffixIn, suffixOut = None, syncronize_stat=False, symmetrize = False, subSamples = [], lnNsubSamples = {}, rescale = None):
        if sampleName in groupPlot:
          subIdx = 0
          for subSample in groupPlot[sampleName]['samples']:
            if subIdx == 0:
              if subSample in subSamples:
                ...
                # scale
                if subSample in lnNsubSamples:
                  if '/' in lnNsubSamples[subSample]:
                    up, down = lnNsubSamples[subSample].split('/')
                    histoUp.Scale(float(up))
                    histoDown.Scale(float(down))
                  else:
                    histoUp.Scale(float(lnNsubSamples[subSample]))
                    histoDown.Scale(1. / float(lnNsubSamples[subSample]))
        ...
        histoDown.SetDirectory(self._outFile)
        if suffixOut:
            if type(suffixOut) == list:
              histoUp.SetName('histo_%s%s' % (sampleName, suffixOut[0]))
              histoDown.SetName('histo_%s%s' % (sampleName, suffixOut[1]))
            else:
