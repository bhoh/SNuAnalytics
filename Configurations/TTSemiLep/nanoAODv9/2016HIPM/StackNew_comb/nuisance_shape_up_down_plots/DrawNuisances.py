...
def DrawNuisances(inputRootFile, inputRootFile_MC, histoNominal, histo_up, histo_down, histoMC, legendName, varName, outputDirPlots = "./", drawYields = "0", drawMC = "0", custom_max_ratio=None ):
 ...
 #    if (den != 0) hRatioUp.SetBinError  (iBin+1, 1./den * hRatioUp.GetBinError(iBin+1))
  ...
  hReferenceRatio.Draw()
  rounded_max_ratio = int( 10. * (max_ratio + 0.3) ) / 10.
  ...
  CMS_lumi.lumi_13TeV = 'L = 36.33/fb'
  ...
  out_file_name = histo_up.replace("Up","").split('/')
  out_file_name = '_'.join(out_file_name)

  name = "%s/cc_%s_Full.png"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
  name = "%s/cc_%s_Full.pdf"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
  name = "%s/cc_%s_Full.root"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
 
  hNominal.GetYaxis().SetRangeUser(0.01,ymax*10.)
  pad1.SetLogy()  
 
  name = "%s/log_cc_%s_Full.png"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
  name = "%s/log_cc_%s_Full.pdf"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
  name = "%s/log_cc_%s_Full.root"%(outputDirPlots, out_file_name)
  ccFull.SaveAs( name )
