...
if with_mass:
  fCombineNameLow ='/cms_scratch/bhoh/toys/fitDiagnosticsPostFit_mass80.root'
  fCombineNameHigh='/cms_scratch/bhoh/toys/fitDiagnosticsPostFit_mass150.root'
else:
  fCombineNameLow ='/cms_scratch/bhoh/toys/fitDiagnosticsPostFit80.root'
  fCombineNameHigh='/cms_scratch/bhoh/toys/fitDiagnosticsPostFit140.root'
...
def getMC(f_, era, cutName, sampleName):
  h_MC = f_.Get(base + "/{VAR1}__{VAR2}/{VAR3}".format(VAR1=era, VAR2=cutName, VAR3=sampleName))
  return h_MC

def getMCs(f_, eras, cutName, sampleName):
  h_MC = None
  for era in eras:
    if h_MC == None:
      h_MC = f_.Get(base + "/{VAR1}__{VAR2}/{VAR3}".format(VAR1=era, VAR2=cutName, VAR3=sampleName))
    else:
      tmp_h_MC = f_.Get(base + "/{VAR1}__{VAR2}/{VAR3}".format(VAR1=era, VAR2=cutName, VAR3=sampleName))
      h_MC.Add(tmp_h_MC)
  return h_MC
...
def saveHistos_CHToCB(fOut, fCombine, cutName, varName):
  ...
  h_Others = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName, 'Others'), h_DATA)
  h_ST     = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName,     'ST'), h_DATA)
  h_TTjj   = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName,  'TT+jj'), h_DATA)
  h_TTcc   = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName,  'TT+cc'), h_DATA)
  h_TTbb   = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName,  'TT+bb'), h_DATA)

  if "sng_4j" in cutName:
    h_QCD    = changeMCbins( getMCs(fCombine, ['Y2018', 'Y2017', 'Y2016noHIPM', 'Y2016HIPM'], cutName,    'QCD'), h_DATA)
...
if __name__ == '__main__':
  # !! SAVE !!
  if base == "shapes_prefit":
    if with_mass:
      fOutLow = ROOT.TFile("results_run2_PreFit_Low_mass.root", "RECREATE")
      fOutHigh = ROOT.TFile("results_run2_PreFit_High_mass.root", "RECREATE")
    else:
      fOutLow = ROOT.TFile("results_run2_PreFit_Low.root", "RECREATE")
      fOutHigh = ROOT.TFile("results_run2_PreFit_High.root", "RECREATE")
  elif base == "shapes_fit_b":
    if with_mass:
      fOutLow = ROOT.TFile("results_run2_PostFit_b_Low_mass.root", "RECREATE")
      fOutHigh = ROOT.TFile("results_run2_PostFit_b_High_mass.root", "RECREATE")
    else:
      fOutLow = ROOT.TFile("results_run2_PostFit_b_Low.root", "RECREATE")
      fOutHigh = ROOT.TFile("results_run2_PostFit_b_High.root", "RECREATE")
  #
  if with_mass:
    ...
    saveHistos_CHToCB(fOutLow, fCombineLow, "sng_4j_eleCH_2b", "DNN_Low")
    saveHistos_CHToCB(fOutLow, fCombineLow, "sng_4j_muCH_2b",  "DNN_Low")
    saveHistos_CHToCB(fOutHigh, fCombineHigh, "sng_4j_eleCH_2b", "DNN_High")
    saveHistos_CHToCB(fOutHigh, fCombineHigh, "sng_4j_muCH_2b",  "DNN_High")
