        ...
        for cut, cat in getSubLists(cuts,'categories') :
          ...
          for variableName, variable in variables.iteritems():
            ...
            for signalName in alternative_signals:
              ...
              card.write(''.join(name.ljust(columndef) for name in ["AllBkgs"]))
              card.write('\n')

              card.write('process'.ljust(80))
              card.write(''.join(('%d' % signal_ids[name]).ljust(columndef) for name in signals))
              card.write(''.join(('%d' % background_ids[name]).ljust(columndef) for name in [cut_backgrounds[0]]))
              card.write('\n')

              card.write('rate'.ljust(80))
              card.write(''.join(('%-.4f' % yieldsSig[name]).ljust(columndef) for name in signals))
              card.write(''.join(('%-.4f' % sum(yieldsBkg[name] for name in cut_backgrounds)).ljust(columndef)))
              card.write('\n')
              ...
              for nuisanceName, nuisance in nuisances.iteritems():
                ...
                if nuisance['type'] in ['lnN', 'lnU']:
                  ...
                  if 'all' in nuisance and nuisance ['all'] == 1: # for all samples
                    totalYieldImpact[entryName] = float(nuisance['value'])
                    card.write(''.join(('%-.4f' % nuisance['value']).ljust(columndef) for _ in [""]))
                  else:
                    # apply only to selected samples
                    totalYieldsBkg   = 0. #sum(yieldsBkg[name] for name in cut_backgrounds)
                    totalYieldsBkgUp = 0.
                    for sampleName in processes:
                      totalYieldsBkg += yieldsBkg[sampleName] if sampleName in cut_backgrounds else 0.
                      if sampleName in nuisance['samples']:
                        totalYieldsBkgUp += yieldsBkg[sampleName]*float(nuisance['samples'][sampleName]) if sampleName in cut_backgrounds else 0.
                        # in this case nuisance['samples'] is a dict mapping sample name to nuisance values in string
                        # weighted average of lnN. sum(yieldsBkg[name] for name in cut_backgrounds)
                      else:
                        totalYieldsBkgUp += yieldsBkg[sampleName] if sampleName in cut_backgrounds else 0.

                    if not totalYieldsBkg == sum(yieldsBkg[name] for name in cut_backgrounds):
                        raise RuntimeError("totalYieldsBkg != sum(yieldsBkg[name] for name in cut_backgrounds)")
                    if totalYieldsBkg > 0.:
                      totalYieldImpact[entryName] = totalYieldsBkgUp/float(totalYieldsBkg)
                      card.write(('%-.4f'%(totalYieldsBkgUp/float(totalYieldsBkg))).ljust(columndef))
                    else:
                      totalYieldImpact[entryName] = '-'
                ...
                if nuisance['type'] in ['shape','shapeN']:
                  ...
                  for sampleName in processes:
                    if ('all' in nuisance and nuisance['all'] == 1) or \
                      ...
                      totalYieldsBkg     += histo.Integral()     if sampleName in cut_backgrounds else 0.
                      totalYieldsBkgUp   += histoUp.Integral()   if sampleName in cut_backgrounds else 0.
                      totalYieldsBkgDown += histoDown.Integral() if sampleName in cut_backgrounds else 0.

                  if totalYieldsBkg > 0.:
                    totalYieldImpact[entryName] = ((totalYieldsBkgUp/float(totalYieldsBkg)),(totalYieldsBkgDown/float(totalYieldsBkg)))
                    card.write((('%-.4f' % (totalYieldsBkgUp/float(totalYieldsBkg)))+"/"+('%-.4f' % (totalYieldsBkgDown/float(totalYieldsBkg)) )).ljust(columndef))
                  else:
                    totalYieldImpact[entryName] = '-'
                    card.write(('-').ljust(columndef))
              ...
              ##############
              groups_yield_nuis = ['jes','jer','btag']
              for group_yield_nuis in groups_yield_nuis: 
                card.write(group_yield_nuis.ljust(80-20))
                card.write(('lnN').ljust(20))
                sum_yieldUp   = 0.
                sum_yieldDown = 0.
                for key, val in totalYieldImpact.iteritems():
                  if group_yield_nuis in key:
                    if type(val) == float:
                      sum_yieldUp   += (log(val))**2
                      sum_yieldDown += (log(val))**2
                    elif type(val) == str:
                      pass
                    elif type(val) == tuple:
                      sum_yieldUp   += (log(val[0]))**2
                      sum_yieldDown += (log(val[1]))**2

                  else:
                    pass
                sum_yieldUp   = exp( sum_yieldUp  **(0.5) )
                sum_yieldDown = exp( sum_yieldDown**(0.5) )
                card.write((('%-.4f' % (sum_yieldUp))+"/"+('%-.4f' % (sum_yieldDown) )).ljust(columndef))
                card.write('\n')
