...
class DatacardFactory:
                  ...
                  if 'AsLnN' in nuisance and float(nuisance['AsLnN']) >= 1.:
                    ...
                    for sampleName in processes:
                      if ('all' in nuisance and nuisance ['all'] == 1) or \
                        ...
                        saved = self._saveNuisanceHistos(cutName, variableName, sampleName, '_' + nuisance['name'], suffixOut, symmetrize, nuisance['samples'], nuisance['lnNsamples'] if 'lnNsamples' in nuisance else {})
                        if saved:
                          card.write('1.000'.ljust(columndef))
                        else:
    ...
    # _____________________________________________________________________________
    def _saveNuisanceHistos(self, cutName, variableName, sampleName, suffixIn, suffixOut = None, symmetrize = False, subSamples = [], lnNsubSamples = {}):
        if sampleName in groupPlot:
          subIdx = 0
          for subSample in groupPlot[sampleName]['samples']:
            if subIdx == 0:
