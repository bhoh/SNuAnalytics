...
if not 'comb' in opt.pycfg:
  if tt_psweight:
    ...
    nuisances['FSR_TT_weight'] = {
        ...
        'kind': 'weight',
        'type': 'shape',
        'samples': dict((skey, fsr_syst) for skey in ttmc if 'CHToCB' in skey),
    
    }
    nuisances['FSR_TT_tree'] = {
        'name': 'ttbar_fsr',
        'kind': 'tree',
        'type': 'shape',
        'samples': dict((skey, ['1.0','1.0']) for skey in ttmc if not 'CHToCB' in skey),
        #flip Up and Down histogram
        'folderDown'   : makeMCDirectory('__FSRup'),
        'folderUp' : makeMCDirectory('__FSRdo'),
    }
else:
  nuisances['FSR_TT'] = {
      'name': 'ttbar_fsr',
      #'rename': ['ttbar_fsrDown','ttbar_fsrUp'], 
      'kind': 'weight',
      'shape': 0.5,
      #'syncronize_stat' : True,
      'type': 'shape',
