...
for rg in ${ARR_REGION[@]};do
	...
	echo "${rg},${proc}"
	mkShapesMulti.py --pycfg=configuration_Boosted_${rg}.py --batchSplit=AsMuchAsPossible --doBatch --treeName Events
	mkShapesMulti.py --pycfg=configuration_Resolved_${rg}.py --batchSplit=AsMuchAsPossible --doBatch --treeName Events
    done
done
