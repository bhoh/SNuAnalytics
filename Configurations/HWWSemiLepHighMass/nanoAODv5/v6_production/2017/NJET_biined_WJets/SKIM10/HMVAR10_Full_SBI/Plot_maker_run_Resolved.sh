...
for flv in ${FLV[@]};do
    ...
    (mkPlot.py --pycfg=configuration_Resolved.py --inputFile=${input} --plotFile=plot_${flv}.py --cutsFile=cuts_Resolved_${flv}.py --outputDirPlots=plots_2017_Resolved_${flv} &> Plot_maker_run_Resolved_${flv}.log;echo "DONE" >> Plot_maker_run_Resolved_${flv}.log)&
done
EndTime=$(date +%s)
echo $EndTime
