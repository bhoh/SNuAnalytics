##Just set below things##


#tag='simple2017'
tag='2017_Resolved_SKIM10_HMVar10_Fullvar___THIS_PROC_____THIS_REGION__'



#variablesFile='variables_limit.py' ##what variables to draw
variablesFile='variables_Resolved.py' ##what variables to draw
cutsFile='cuts_Resolved___THIS_PROC_____THIS_REGION__.py' ## event selection##region selection
#plotFile='plot.py' ##color code and some format-related things

samplesFile = 'samples_2017.py'

lumi=41.5

#outputDirPlots='plots_'+tag+'Blined'
outputDirPlots='plots_'+tag
outputDir =           'rootFile_'+tag
treeName='Events'
aliasesFile='aliases.py'
nuisancesFile = 'nuisances_Resolved.py'

#structureFile = 'structure.py'


#outputDirDatacard='DataCards'

maxLogCratio=10000
