...
#PROCLIST=(WW WWJJ WWW WWZ WZ WZZ ZZZ ZZ)
...
for bst in ${ARR_BOOST[@]};do
    ...
    python python_tool/ExportShellCondorSetup.py -c "cd ${CURDIR};mkdir -p $rootfiledir/hadddir_${COMBINED_PROC}/temp/;hadd -j 10 -d $rootfiledir/hadddir_${COMBINED_PROC}/temp/ -f $rootfiledir/hadddir_${COMBINED_PROC}/plots_${COMBINED_PROC}.root ${haddlist};" -d $rootfiledir/hadddir_${COMBINED_PROC}/ -n "Make${COMBINED_PROC}Shape${bst}" -m 10 -s
