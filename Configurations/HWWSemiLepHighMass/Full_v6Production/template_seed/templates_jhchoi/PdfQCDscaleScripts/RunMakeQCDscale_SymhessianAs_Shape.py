...
def SubmitMakeEnvelopShape(nuisance,s,bst,reg):
    ...
    for f in filelist:
        command+='python PdfQCDscaleScripts/MakeEnvelopShape.py -c '+configuration+' -n '+nuisance+' -s '+s+' -f '+f+'&&'
    command+="echo '--'"
    print command
...
def SubmitMakeSymhessianAsShape(nuisance,s,bst,reg):
    ...
    command='cd '+os.getcwd()+';'
    for f in filelist:
        command+='python PdfQCDscaleScripts/MakeSymhessianAsShape.py -c '+configuration+' -n '+nuisance+' -s '+s+' -f '+f+';'
    print command
...
def SubmitMakeRmsShape(nuisance,s,bst,reg):
    configuration='configuration_'+bst+'_'+reg+'.py'
    #nuisance='QCDscaleAccept'
    filelist=glob.glob('rootFile*_*'+bst+'*_*'+reg+'*/*_'+s+'.*root')
    command='cd '+os.getcwd()+';'
    for f in filelist:
        command+='python PdfQCDscaleScripts/MakeRmsShape.py -c '+configuration+' -n '+nuisance+' -s '+s+' -f '+f+';'
    print command
        ...
        ##s='top'
            for bst in BST_LIST:
                for reg in REGION_LIST:
                    ...
                    nuisance='pdfAccept'
                    if Year=='2016':
                        SubmitMakeRmsShape(nuisance,s,bst,reg)
                    else:
