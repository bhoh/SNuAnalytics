...
#List_SampleTemplate=['samples_2018limit_MassTemplate_ele.py','samples_2018limit_MassTemplate_mu.py']
...
for MX in List_MX_common:
    for flv in ['ele','mu']:
        ...
        ##SampleTemplate
        for rg in ['SR','CR']:
            f=open('samples_2018limit_M'+str(MX)+'_'+flv+'_'+rg+'.py','w')
            for s in BKG:
                f.write('samples["'+s+'"]={}\n')
            if 'ele' in flv:
